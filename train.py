...
parser.add_argument('--data_root', default='F:/Reserch/codes/CMS-LSTM/data/Moving_MNIST',
...
parser.add_argument('--test_times', type=int, default=50, help='Test Times for one epoch')
parser.add_argument('--save_n_epoch', type=int, default=10, help='Save per n epoch')
...
parser.add_argument('--patience', type=int, default=2, help='early_stopping_patience')
...
if __name__ == '__main__':
    ...
    for epoch in range(trainer.start_epoch, opt.total_epoch):
        ...
        t_epoch = epoch+1
        if t_epoch % opt.save_n_epoch == 0:
            with torch.no_grad():
                trainer.detail_test(t_epoch)
